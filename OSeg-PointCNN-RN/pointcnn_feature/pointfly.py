...
def batch_normalization(data, is_training, name, reuse=None, name_scope=''):
...
def separable_conv2d(input, output, name, is_training, kernel_size, depth_multiplier=1,
                     reuse=None, with_bn=True, activation=tf.nn.elu, name_scope=''):
    if name_scope:
        name = name_scope + '/' + name
...
def dense(input, output, name, is_training, reuse=None, with_bn=True, activation=tf.nn.elu, name_scope=''):
