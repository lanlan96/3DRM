...
def xconv(pts, fts, qrs, tag, N, K, D, P, C, C_pts_fts, is_training, with_X_transformation, depth_multiplier,
          sorting_method=None, with_global=False, reuse=None, name_scope=''):
    ...
    nn_fts_from_pts_0 = pf.dense(nn_pts_local, C_pts_fts, tag + 'nn_fts_from_pts_0', is_training, reuse=reuse, name_scope=name_scope)
    nn_fts_from_pts = pf.dense(nn_fts_from_pts_0, C_pts_fts, tag + 'nn_fts_from_pts', is_training, reuse=reuse, name_scope=name_scope)
    if fts is None:
    ...
    if with_X_transformation:
        ######################## X-transformation #########################
        X_0 = pf.conv2d(nn_pts_local, K * K, tag + 'X_0', is_training, (1, K), reuse=reuse, name_scope=name_scope)
        X_0_KK = tf.reshape(X_0, (N, P, K, K), name=tag + 'X_0_KK')
        X_1 = pf.depthwise_conv2d(X_0_KK, K, tag + 'X_1', is_training, (1, K), reuse=reuse, name_scope=name_scope)
        X_1_KK = tf.reshape(X_1, (N, P, K, K), name=tag + 'X_1_KK')
        X_2 = pf.depthwise_conv2d(X_1_KK, K, tag + 'X_2', is_training, (1, K), activation=None, reuse=reuse, name_scope=name_scope)
    ...
    fts_conv = pf.separable_conv2d(fts_X, C, tag + 'fts_conv', is_training, (1, K), depth_multiplier=depth_multiplier, reuse=reuse, name_scope=name_scope)
    fts_conv_3d = tf.squeeze(fts_conv, axis=2, name=tag + 'fts_conv_3d')

    if with_global:
        fts_global_0 = pf.dense(qrs, C // 4, tag + 'fts_global_0', is_training, reuse=reuse, name_scope=name_scope)
        fts_global = pf.dense(fts_global_0, C // 4, tag + 'fts_global', is_training, reuse=reuse, name_scope=name_scope)
...
class PointCNN:
    def __init__(self, points, features, is_training, setting, name_scope='', reuse=None):
        ...
        if setting.sampling == 'fps':
            from pointcnn_feature.sampling import tf_sampling
        ...
        if features is None:
            ...
            features_hd = pf.dense(features, C_fts, 'features_hd', is_training, reuse=reuse, name_scope=name_scope)
        ...
        for layer_idx, layer_param in enumerate(xconv_params):
            ...
            fts_xconv = xconv(pts, fts, qrs, tag, N, K, D, P, C, C_pts_fts, is_training, with_X_transformation,
                              depth_multiplier, sorting_method, with_global, reuse=reuse, name_scope=name_scope)
            fts_list = []
            for link in links:
        ...
        if hasattr(setting, 'xdconv_params'):
            for layer_idx, layer_param in enumerate(setting.xdconv_params):
                ...
                fts_xdconv = xconv(pts, fts, qrs, tag, N, K, D, P, C, C_pts_fts, is_training, with_X_transformation,
                                   depth_multiplier, sorting_method, reuse=reuse, name_scope=name_scope)
                fts_concat = tf.concat([fts_xdconv, fts_qrs], axis=-1, name=tag + 'fts_concat')
                fts_fuse = pf.dense(fts_concat, C, tag + 'fts_fuse', is_training, reuse=reuse, name_scope=name_scope)
                self.layer_pts.append(qrs)
        ...
        for layer_idx, layer_param in enumerate(self.fc_params):
            ...
            fc = pf.dense(self.fc_layers[-1], C, 'fc{:d}'.format(layer_idx), self.is_training, reuse=reuse, name_scope=name_scope)
            fc_drop = tf.layers.dropout(fc, dropout_rate, training=self.is_training, name='fc{:d}_drop'.format(layer_idx))
